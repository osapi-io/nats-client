...
1. **Migrate GET requests to NATS**:
   - Modify API to **publish GET requests to a NATS subject**
   - Workers process the request and **store responses in KV**
   - API reads results **from KV instead of a direct response**

2. **Implement Subject-Based Worker Routing**:
   - The **worker listens to `jobs.query.*` for read operations**
   - The **worker listens to `jobs.modify.*` for write operations**
   - Ensure **read requests store responses in KV**

3. **Validate Worker Performance**:
   - Measure **processing latency** and **message retention** in the KV store

4. **Deploy in Staging & Monitor**:
   - Ensure API correctly retrieves **delayed responses**
